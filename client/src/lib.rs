...
pub static PLAYER_ID: Lazy<Arc<Mutex<Option<u64>>>> = 
    Lazy::new(|| Arc::new(Mutex::new(None)));
...
pub fn create_input_bytes(
    ...
    let input = PlayerInput {
        player_id,
        carrier_direction: if has_dir { Some([carrier_dir_x, carrier_dir_y]) } else { None },
...
#[wasm_bindgen]
pub fn render_frame(ctx: &web_sys::CanvasRenderingContext2d, canvas_width: f64, canvas_height: f64) {
    let state = {
    ...
    if let Some(state) = state {
        ...
        for pos in &state.asteroid_positions {
            ctx.begin_path();
            let _ = ctx.arc(pos[0] as f64 * scale_x, pos[1] as f64 * scale_y, 4.0 * scale_x, 0.0, std::f64::consts::PI * 2.0);
            ctx.fill();
        }
        ...
        for pos in &state.alien_positions {
            ...
            ctx.fill_rect((pos[0] as f64 * scale_x) - size/2.0, (pos[1] as f64 * scale_y) - size/2.0, size, size);
        }
        
        // Draw Carrier (blue circle)
        ctx.set_fill_style_str("#3388ff");
        ctx.begin_path();
        let _ = ctx.arc(state.carrier_pos[0] as f64 * scale_x, state.carrier_pos[1] as f64 * scale_y, 10.0 * scale_x, 0.0, std::f64::consts::PI * 2.0);
        ctx.fill();
        ...
        for pos in &state.boid_positions {
            ...
            ctx.fill_rect((pos[0] as f64 * scale_x) - 1.0, (pos[1] as f64 * scale_y) - 1.0, 2.0, 2.0);
        }
